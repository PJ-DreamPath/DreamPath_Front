...
function TicketPurchasePage(props) {
    ...
    useEffect(() => {
        if (!searchTicketPurchaseList.isLoading) {
            ...
            const totalPages = searchTicketPurchaseList?.data?.data.totalPages || 1;
    ...
    useEffect(() => {
        ...
        console.log(searchTicketPurchaseList);
    }, [searchParams])
        ...
        <div css={s.container}>
            ...
            <div css={s.tableWrapper}>
                <table css={s.table}>
                    ...
                    <tbody>
                        {  
                            searchTicketPurchaseList?.data?.data.ticketPurchaseHistoryList.map((ticket, index) => (
                                <tr key={index} css={s.tableRow}>
                                    <td css={s.tableCell}>{ticket.ticketName}</td>
                                    <td css={s.tableCell}>{Number(ticket.price).toLocaleString()}P</td>
                                    <td css={s.tableCell}>{ticket.createdAt}</td>                      
                                </tr>
                            ))
                        }
                    </tbody>
            ...
            <div css={s.footer}>
                <div css={s.pageNumbers}>
                    <button disabled={searchTicketPurchaseList?.data?.data.firstPage} onClick={() => handlePageNumbersOnClick(page - 1)}><GoChevronLeft /></button>
                    {
                        pageNumbers.map(number => 
                            <button key={`ticket${number}`} css={s.pageNum(page === number)} onClick={() => handlePageNumbersOnClick(number)}><span>{number}</span></button>
                        )
                    }
                    <button disabled={searchTicketPurchaseList?.data?.data.lastPage} onClick={() => handlePageNumbersOnClick(page + 1)}><GoChevronRight /></button>
                </div>
            </div>
        </div>
